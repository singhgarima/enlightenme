...
@click.option('--output', '-o', required=False, help="Write to FILE instead of stdout")
@click.option('--keywords', '-k', required=False,
              help="Topics or Keywords you are interested in (e.g. python,golang,chaos)")
@click.pass_context
def source(ctx: click.Context, source_name: str, format: str, output: str = None, keywords: str = None):
    if source_name == "list":
        list_sources()
    else:
        keywords = keywords.split(",") if keywords else None
        news_fetchers = fetcher.Fetcher(source_name, format_type=format, keywords=keywords)
